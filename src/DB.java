...
public class DB {
	...
	private static final String CONNECTION_STRING = "jdbc:mysql://127.0.0.1/project";
	...
	public void addSentence(String sentence, int heading)
	{
			...
			{
				System.out.println("Error: DB.java - Couldn't clear the table. Function addSentence() Line 118");
				System.out.println(e.toString());
				e.printStackTrace();
			}
	}
	...
	public void getResultSet(float score)
	{
		...
		try 
		{
			...
			if(score == -1)
				...
				concat_statement = "SELECT * FROM sentence_table WHERE score > " + score + " OR head = 1";
			rs = stmt.executeQuery(concat_statement);
		} 
		catch (SQLException e) 
		{
			System.out.println("Error: DB.java - Couldn't clear the table. Function getResultSet() Line 139");
			e.printStackTrace();
		}
	}
