...
public class MainApp {
	public static DB db = new DB("root", "12345");
	...
	public MainApp(String chosenFile) throws SQLException, SAXException, ParserConfigurationException
	{
		...
		try 
		{
			...
			while((sent = db.getNextSentence())!=null)
			{
				...
				if(SampleUi.text!=null && sent.getSentence().toLowerCase().contains(SampleUi.text.trim().toLowerCase()))
				{
					sent.setScore(score+900);
					System.out.println("Cue Phrase");
				}
				else
					sent.setScore(score);
				db.updateScore(sent);
			...
			compression_ratio = SampleUi.getSliderValue();
	...
	private static void splitSentences(String source) throws IOException, SAXException, ParserConfigurationException 
	{
		...
		numberOfSentences=0;
		String toadd;
		db.connectDB("com.mysql.jdbc.Driver");
